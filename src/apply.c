...
struct obj *obj;
{
   ...
   char *typeword;
   if (!freehand()) {
	  You("have no %s free to write with.", body_part(HAND));
	  return 0;
   } else if (Blind) {
	  pline("Writing a scroll while blind doesn't seem like a good idea.");
	  return 0;
	  }
   if (obj->spe == 0){
   ...
   target = getobj(write_on, "write on");
   if (!target){
	   return 0;
   }
   if (target->otyp == SPE_NOVEL){
	   pline("Terry Pratchett would not be proud.");
	   return 0;
   }
   typeword = (target->oclass == SPBOOK_CLASS) 
	   ? "spellbook" 
	   : "scroll";
   ...
   else if (target->cursed || target->blessed){
	   new_obj->cursed = target->cursed;
	   new_obj->blessed = target->blessed;
   }
   ...
   if (new_obj->oclass == SPBOOK_CLASS && new_obj->otyp != SPE_BLANK_PAPER){
	  pline_The("spellbook changes."); /* too lazy */
   }
   useup(target);
#ifdef MAIL
